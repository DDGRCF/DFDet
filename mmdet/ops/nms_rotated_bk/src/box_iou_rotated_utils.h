...
template <typename T>
HOST_DEVICE_INLINE void get_poly_vertices(
    const PolyBox<T>& box,
    Point<T> (&pts)[4]) {
...
template <typename T>
HOST_DEVICE_INLINE T polygon_area(const PolyBox<T> & poly) {
  T area = 0;
  Point<T> pts[4];
  pts[0].x = poly.x0; pts[0].y = poly.y0;
  pts[1].x = poly.x1; pts[1].y = poly.y1;
  pts[2].x = poly.x2; pts[2].y = poly.y2;
  pts[3].x = poly.x3; pts[3].y = poly.y3;
  // for (int i = 1; i < 3; i++) {
  //   area += fabs(cross_2d<T>(pts[i] - pts[0], pts[i + 1] - pts[0]));
  // }
  area += fabs(cross_2d<T>(pts[1] - pts[0], pts[2] - pts[0]));
  area += fabs(cross_2d<T>(pts[2] - pts[0], pts[3] - pts[0]));
...
template <typename T>
HOST_DEVICE_INLINE T rotated_boxes_intersection(
  const PolyBox<T>& box1,
  const PolyBox<T>& box2) {
  Point<T> intersectPts[24], orderedPts[24];

  Point<T> pts1[4];
  Point<T> pts2[4];
  get_poly_vertices<T>(box1, pts1);
  get_poly_vertices<T>(box2, pts2);
...
template <typename T>
HOST_DEVICE_INLINE T
single_poly_iou_rotated(T const* const box1_raw, T const* const box2_raw) {
  // shift center to the middle point to achieve higher precision in result
  PolyBox<T> box1, box2;
  // box1.x0 = box1_raw[0];
  // box1.y0 = box1_raw[1];
  ...
  T area1 = polygon_area<T>(box1);
  T area2 = polygon_area<T>(box2);
  if (area1 < 1e-14 || area2 < 1e-14) {
    return 0.f;
  }
