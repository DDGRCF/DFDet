...
def arb2result_withmask(bboxes, masks, labels, num_classes, img_meta, bbox_type='hbb'):
    assert bbox_type in ['hbb', 'obb', 'poly']
    bbox_dim = get_bbox_dim(bbox_type, with_score=True)
    
    if bboxes.shape[0] == 0:
        bbox_results = [
            np.zeros((0, bbox_dim), dtype=np.float32) for _ in range(num_classes)
        ]
        mask_results = [
            np.zeros((0, masks.shape[-1]), dtype=np.float32) for _ in range(num_classes)
        ]
        ...
        labels = labels.cpu().numpy()
        mask_results = [masks[labels == i] for i in range(num_classes)]
...
def distance2obb(points, distance, max_shape=None):
    ...
    Matrix = torch.cat([Cos, Sin, -Sin, Cos], dim=1).view(*theta.size()[:-1], 2, 2)

    wh = distance[..., :2] + distance[..., 2:]
    offset_t = (distance[..., 2:] - distance[..., :2]) / 2
    offset_t = offset_t.unsqueeze(-1)
    offset = torch.matmul(Matrix, offset_t).squeeze(-1)
    ctr = points + offset
...
def distance2mask(points, distance, polar_offset, max_shape=None):
    # points [n, 1, 2] distance [n, 36] polar_offset [1, 36]
    polar_offset = polar_offset.view(1, -1)
    Cos, Sin = torch.cos(polar_offset), torch.sin(polar_offset)
    Matrix = torch.stack([Cos, -Sin], dim=-1) # [1, 36, 2]
    mask_points = distance[..., None] * Matrix + points[:, None] # [n, 36, 2]
    return mask_points
