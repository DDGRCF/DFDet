...
class OBBATSSCostV2Assigner(BaseAssigner):
    ...
    def __init__(self, topk, 
                 alpha=0.5, wh_thre = -1, iou_calculator=dict(type='OBBOverlaps'),
                 ignore_iof_thr=-1):
        self.topk = topk
        self.alpha = alpha
    ...
    def assign(self,
        ...
        with torch.no_grad():
            ...
            bbox_preds_ctr_xy = bbox_preds[:, :2]
            bbox_preds_wh = bbox_preds[:, 2:4]
            bbox_preds_lt = bbox_preds_ctr_xy - bbox_preds_wh / 2
            bbox_preds_rb = bbox_preds_ctr_xy + bbox_preds_wh / 2
            overlaps = self.iou_calculator(torch.cat((bbox_preds_lt, bbox_preds_rb), 
                    dim=1), gt_bboxes, mode="giou")
            overlaps = torch.maximum(overlaps, overlaps.new_tensor(0.))
            # overlaps = torch.pow(overlaps, 2/3)

            # compute cls cost for bbox and gt
            cls_cost = torch.sigmoid(cls_scores[:, gt_labels])

            # theta gaps
            theta_cost = 1 - self.calculator_theta_gaps(bbox_preds[:, -1], gt_bboxes[:, -1])

        assert cls_cost.shape == overlaps.shape

        overlaps = cls_cost ** (1 - self.alpha) * (overlaps * theta_cost).sqrt() ** self.alpha
    ...
    def calculator_theta_gaps(self, pred_thetas, gt_thetas):
        pred_thetas = pred_thetas[:, None]
        gt_thetas = gt_thetas[None, :]
        thre = numpy.pi / 4
        diff = torch.abs(pred_thetas - gt_thetas) #  (n_p, n_g)
        diff = torch.where(diff > thre, thre - diff, diff)
        return diff / thre