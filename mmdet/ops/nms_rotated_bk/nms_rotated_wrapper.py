...
def poly_nms(dets, iou_thr, device_id=None):
    ...
    if dets_th.numel() == 0:
        ...
        # same bug will happen when bboxes is too small
        too_small = (dets_th[:, 0:8:2].max(1)[0] - dets_th[:, 0:8:2].min(1)[0]) \
            * (dets_th[:, 1:8:2].max(1)[0] - dets_th[:, 1:8:2].min(1)[0]) < 0.001
        if too_small.all():
            inds = dets_th.new_zeros(0, dtype=torch.int64)
        else:
            ori_inds = torch.arange(dets_th.size(0))
            ...
            bboxes, scores = dets_th[:, :8], dets_th[:, 8]
            inds = nms_rotated_ext.nms_poly(bboxes, scores, iou_thr)
            inds = ori_inds[inds]
